...
UNTIL0
DEFPROCinit
LOCALa%:FX%=&AB8:FY%=&AC0:FH%=&AC8:FV%=&AD0:F%=&AD8:S%=0
BX%=&AE0:BY%=&AE8:B%=&AF0:lev%=1:DIMs%(3)
FORa%=0TO3:READs%(a%):NEXT
DATA134,133,143,131
ENDPROC
DEFPROCoff
VDU4,12:FORZ=0TO15:VDU19,Z,0;0;:NEXT
M$="GET READY!":IFlev%>4:M$="OH BOY...!":IFlev%>8:M$="AAAAAAAGH!"
VDU19,14,7;0;17,14,31,5,15:PRINT;M$;
ENDPROC
DEFPROCon
...
ENDPROC
DEFPROCstart
DX%=32:DY%=214:DZ%=1:PROCd(DZ%,DX%,DY%)
...
ENDPROC
DEFFNbite(i%,j%)
IFi%<DX% OR i%>(DX%+12) OR j%<(DY%-16) OR j%>DY%:=FALSE:ELSE:=TRUE
DEFPROCfcheck(X%,Y%,E%)
C%=FNrc(X%,Y%)
...
PROCd(F%?E%,FX%?E%,FY%?E%):F%?E%=F%?E%+4:PROCd(F%?E%,FX%?E%,FY%?E%)
...
DEFPROCbon
B=0:FORA=0TO7
IFFH%?A<>2ANDFY%?A<>0:SOUND2,-10,(30+B*10),1:S%=S%+50:B=B+1:PROCscore:VDU18,0,1,25,4,FX%?A*16;(FY%?A*4)+40;238
FORz=1TO500:NEXT
NEXT
ENDPROC
DEFPROCover
