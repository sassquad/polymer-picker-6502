...
IFq%?E%<>0IFl%>4IFFNfb(k%?E%,n%?E%)PROCu
E%=(E%+1)MOD8:ENDPROC
DEFPROCE:IFV%=1IFST%<>1ST%=(ST%+1)AND1:ENDPROC
OS%=SX%:OY%=SY%:OA%=SD%:IFSX%<D%ANDSX%<62SX%=SX%+V%:SD%=5:IFFNsbr(SX%+12)PROCu
IFSX%>D%ANDSX%>1SX%=SX%-V%:SD%=4:IFFNsb(SX%+4)PROCu
IFSX%=D%SD%=8:IFFNsb(SX%+8)PROCu
IFSY%<e%ANDSY%<194SY%=SY%+V%
IFSY%>e%ANDSY%>64SY%=SY%-V%
IFOS%<>SX%OROY%<>SY%OROA%<>SD%PROCd(OA%,OS%,OY%):PROCd(SD%,SX%,SY%)
IFV%=1ST%=(ST%+1)MOD2:ENDPROC
...
DEFPROCB:PRINTD$;P%;:IF(l%MOD2)=1ORl%>8VDU31,6,30,17,6,241,32,17,7:PRINT;L%
VDU17,128,5:ENDPROC
...
DEFPROCu:M%=150:PROCD:V%=2:SOUND2,2,148,4:VDU18,3,1,25,4,(D%*16)+RND(64);(e%*4)+RND(32);239;:ENDPROC
...
DEFPROCG:*FX21,6
SOUND3,2,96,4:PROCw:VDU4,17,co%,12,31,5,15:PRINT"GAME  OVER":PROChi:PROCw:ENDPROC
...
NEXT:ENDPROC
DEFPROChi:RESTORE113:FORu=1TO8:READn,d:SOUND1,1,n,d:NEXT:ENDPROC
...
113DATA53,5,41,10,53,10,73,15,41,5,33,10,53,10,69,20
