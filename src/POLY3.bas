...
DEFPROCA:RESTORE(95+(l%MOD4)):READco%:VDU28,0,4,19,0,17,co%,12,26,5:READci%,m%:IFci%=1READc1%,c2%,c3%,c4%:PROCc(c1%,c2%,c3%,c4%)
IFci%=2:FORa=1TO2:READc1%,c2%,c3%,c4%:PROCc(c1%,c2%,c3%,c4%):NEXT
IFm%=1READc1%,c2%,c3%,c4%:PROCH(c1%,c2%,c3%,c4%)
IFm%>1FORa=1TOm%:READc1%,c2%,c3%,c4%:PROCH(c1%,c2%,c3%,c4%):NEXT
...
DEFPROCa:D%=32:e%=214:f%=1:PROCd(f%,D%,e%):P%=8:L%=8:O%=1224:OD%=300:SF%=1:g%=1:OX%=0:OB%=0:IF(l%MOD2)=1cr%=228:co%=1:ELSEcr%=229:co%=5
...
IFSY%<e%ANDSY%<194SY%=SY%+SF%
IFSY%>e%ANDSY%>64SY%=SY%-SF%
...
IFO%<1068:IFOX%=0ANDOB%=0SOUND2,3,240,2:VDU18,3,6,25,4,612;850;237;:OB%=1
TIME=0:ENDPROC
...
DEFPROCu:OD%=150:PROCD:SF%=2:SOUND2,2,148,4:VDU18,3,1:FORz=1TO4:PLOT69,(D%*16)+RND(64),(e%*4)+RND(64):NEXT:ENDPROC
DEFPROCF:IF(l%MOD2)=1ORl%>8PROCM
PROCp:PROCw:ENDPROC
DEFPROCM:B=0:FORA=0TO1:IFq%?A<>2ANDn%?A<>0SOUND2,1,(30+B*10),1:S%=S%+50:B=B+1:PROCe:VDU18,0,1,25,4,k%?A*16;(n%?A*4)+40;238
...
RESTOREs%:d%=1:FORu=1TO7:READn,C:C=C+2:SOUNDd%,1,n,C:FORT=1TO400:NEXT:d%=d%+1:IFd%=4d%=1
NEXT:ENDPROC
...
DATASTEVE,NUNI,ANDON,EDORA,GRAEME,EWOK,KIT-KAT,ROCKY
