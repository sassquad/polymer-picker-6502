...
REPEAT:PROCdiver:PROCair
IFlev%>8:PROCmf:PROCms:ELSEIF(lev%MOD2)=1:PROCmf ELSE PROCms
PROCmsl
UNTIL P%=0 OR L%=0 OR O%<928
IFP%=0 AND O%>927 PROCgreat:lev%=lev%+1
UNTIL L%=0 OR O%<928
PROCover:PROCtable(S%):UNTIL0
DEFPROCinit:FX%=&AB8:FY%=&AC0:FH%=&AC8:FV%=&AD0:F%=&AD8:BX%=&AE0:BY%=&AE8:S%=0:lev%=1:DIMHS%(10),H$(10):FORI%=0TO9:HS%(I%)=5000:H$(I%)="FRED"+STR$I%:NEXT:ENDPROC
...
VDU19,0,4;0;19,15,0;0;19,14,6;0;5:READo%,p%:VDU19,8,o%;0;19,9,p%;0;:ENDPROC
DEFPROCsc:da%=127+(lev%MOD4):RESTOREda%:READco%:VDU28,0,4,19,0,17,co%,12,26,5:READci%,mt%
...
TIME=0:REPEATUNTILTIME=200:VDU28,0,26,19,5,17,co%,12,26,31,5,6:PRINT"GAME  OVER":TIME=0:REPEATUNTILTIME=300:ENDPROC
...
DEFPROCt:IFlev%>8:re%=126:ELSEIF(lev%MOD2)=1:re%=124 ELSE re%=125
...
DEFPROCtable(sc%):*FX15
Z=-1:REPEAT:Z=Z+1:UNTILsc%>HS%(Z)OR Z=9:FORJ%=9TO Z+1 STEP-1:HS%(J%)=HS%(J%-1):H$(J%)=H$(J%-1):NEXT
IFZ<9PRINTTAB(2,21)"Enter your name:";:INPUTTAB(6,27)n$:H$(Z)=LEFT$(n$,8):HS%(Z)=sc%
@%=5:FOR I%=0 TO 9:VDU31,1,I%*2+6
PRINTI%+1") "HS$(I%)SPC(10-POS);HS%(I%)
NEXT:*FX15
REPEATUNTILGET=32:CLS:ENDPROC
DEFPROCr:*T.
*FX138,0,128
*FX3,2
