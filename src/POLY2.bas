...
N%=RND(-TIME):PROCinit
MODE2:VDU23;8202;0;0;0;:HIMEM=&2C00
REPEAT:REPEAT
...
VDU4,28,0,31,19,27,17,143,12,26,28,0,28,19,27,17,132,12,26,5
...
DEFPROCon:VDU4,17,128,31,5,15:PRINTSPC(10):FORZ=0TO15:VDU19,Z,Z;0;:NEXT:VDU19,0,4;0;19,15,0;0;5
...
DEFPROCstart
X%=32:Y%=214:Z%=1:PROCdraw(Z%,X%,Y%)
BW%=7:BH%=7:V%=2:H%=1:P%=8:L%=8:O%=1224:OD%=200:SF%=1:DF%=1:OXY%=0
IF(lev%MOD2)=1cr%=228:co%=1:ELSE:cr%=229:co%=5
crx%=FNgrx:VDU5,18,3,co%,25,4,crx%;192;cr%
ENDPROC
DEFPROCmovesealife
...
DEFPROCfish:FORA=0TO7:FX%?A=FNrx:FY%?A=FNry:FH%?A=3:F%?A=3
IFA>3:FH%?A=1:F%?A=2
...
FORA=0TO7:BX%?A=(RND(1100)+80)/8:BY%?A=(RND(600)+200)/4:VDU18,3,1,25,4,BX%?A*8;BY%?A*4;251:NEXT
ENDPROC
DEFPROCairbar:VDU5,18,0,14:MOVE920,40:MOVE1216,40:PLOT85,920,52:PLOT85,1216,52:VDU18,0,7:MOVE912,36:DRAW1224,36:DRAW1224,56:DRAW912,56:DRAW912,36:ENDPROC
DEFPROCair
IFTIME>OD%:O%=O%-8:SOUND0,4,6,3:IFO%>912 VDU18,0,15,25,4,O%;52;25,5,O%;40;:TIME=0
IFO%>1000 VDU19,14,6;0; ELSE VDU19,14,9;0;
ENDPROC
DEFPROCmoveshark
OSX%=SX%:OSY%=SY%:OSD%=SD%
IFSX%<X%ANDSX%<66:SX%=SX%+SF%:SD%=5
IFSX%>X%ANDSX%>0:SX%=SX%-SF%:SD%=4
...
IF FX%?E%<4:FH%?E%=3:F%?E%=3
IF FX%?E%>75:FH%?E%=1:F%?E%=2
IF FY%?E%<64 AND FH%?E%<>2 THEN FV%?E%=8 ELSE IF FY%?E%<48 AND FH%?E%=2 THEN FY%?E%=48
IF FY%?E%>200 AND FH%?E%<>2:FV%?E%=4
...
IF FNrs(CX%,CY%)=5 OR FNrs(CX%+1,CY%)=5 OR FNrs(CX%+2,CY%)=5 OR FNrs(CX%+3,CY%)=5 OD%=50:SF%=2:PROChurt ELSE OD%=200
...
DEFPROCcheck(X%,Y%):CALLQ%:C%=?&AB2
IFC%<8:P%=P%-1:S%=S%+200:VDU4:PRINTTAB(13-LENSTR$S%,0);S%:SOUND1,1,99,1:VDU5,18,3,1,25,4,BX%?C%*8;BY%?C%*4;251:SOUND0,-10,150,1:BX%?C%=255:BY%?C%=255
PROCstatus:ENDPROC
DEFPROCfishcheck(X%,Y%,E%):CALLQ%:C%=?&AB2
IFC%<8:PROCdeadfish(E%):VDU18,3,1,25,4,BX%?C%*8;BY%?C%*4;251:SOUND0,-10,150,1:BX%?C%=255:BY%?C%=255
ENDPROC
DEFPROCdeadfish(E%):P%=P%-1:L%=L%-1:SOUND0,-10,50,10:REMPROCdraw(F%?E%,FX%?E%,FY%?E%)
FH%?E%=2:FV%?E%=0:PROCstatus:ENDPROC
DEFPROCstatus:VDU26,4,17,143,17,1,31,1,30,251,32,17,7:PRINT;P%
