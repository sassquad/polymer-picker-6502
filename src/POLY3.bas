...
PROCg:PROCB:PROCe:PRINTA$:TIME=0:REPEAT:PROCv:PROCD:PROCcj:IFl%>6PROCm:PROCE:ELSEIFl%AND1PROCm:ELSE:PROCE
...
DEFPROCa:D%=32:e%=214:f%=1:PROCd(f%,D%,e%):P%=8:L%=8:O%=1224:M%=300:V%=1:g%=1:E%=0:ee%=0:H%=0:K%=0:ST%=0:IF(l%MOD2)=1cr%=228:co%=1:ELSEcr%=229:co%=5
...
DEFPROCs:SD%=4:SX%=FNr(4):SY%=60+(l%*10):IFSX%<=(D%+8)SD%=5:ELSEIFSY%>140SY%=140
...
ENDPROC
DEFPROCg:FORA=0TO7:G%?A=(A+1)*16:J%?A=((RND(136)-1)+64):VDU18,3,7,25,4,G%?A*8;J%?A*4;(247+l%MOD4):NEXT:ENDPROC
DEFPROCB:PRINTD$;P%;:IF(l%MOD2)=1ORl%>6VDU31,6,30,17,6,241,32,17,7,(48+L%)
VDU17,128,5:ENDPROC
...
IFD%>30IFD%<40IFe%>210IFO%<1068IFH%=0:PROCJ
IFINKEY-56:REPEATUNTILINKEY-54
IFINKEY-17:*FX210,1
IFINKEY-82:*FX210,0
ENDPROC
DEFPROCK(c%):VDU18,3,7,25,4,G%?C%*8;J%?C%*4;(247+l%MOD4),4:P%=P%-1:S%=S%+200:G%?C%=0:J%?C%=0:PRINTTAB(13-LENSTR$S%,0);S%:ENDPROC
DEFPROCL(E%,C%):P%=P%-1:L%=L%-1:SOUND3,2,96,4:PROCd(F%?E%,k%?E%,n%?E%):F%?E%=F%?E%+4:PROCd(F%?E%,k%?E%,n%?E%):q%?E%=2:t%?E%=0:PROCB:VDU18,3,7,25,4,G%?C%*8;J%?C%*4;(247+l%MOD4):G%?C%=0:J%?C%=0:ENDPROC
...
DEFPROCu:M%=50:PROCD:SOUND2,2,148,4:VDU18,3,1,25,4,(D%*16)+RND(64);(e%*4)+RND(32);239;:ENDPROC
...
DEFPROCj:FORo=0TO3:U%?o=FNr(1):zz%?o=(RND(101)-1)+96:PROCd(9,U%?o,zz%?o):NEXT:ENDPROC
DEFPROCcj:ee%=(ee%+1)MOD4:IFFNf(U%?ee%,zz%?ee%)PROCu
ENDPROC
