...
PROCover
UNTIL0
DEFPROCinit:LOCALa%:FX%=&9C8:FY%=&9D0:FH%=&9D8:FV%=&9E0:F%=&9E8:BX%=&9F0:BY%=&9F8:S%=0:lev%=1:DIMs%(3)
REMDEFPROCinit:LOCALa%:FX%=&AB8:FY%=&AC0:FH%=&AC8:FV%=&AD0:F%=&AD8:BX%=&AE0:BY%=&AE8:B%=&AF0:S%=0:lev%=1:DIMs%(3)
FORa%=0TO3:READs%(a%):NEXT
DATA134,133,143,131
ENDPROC
DEFPROCoff:VDU4,12:REMFORZ=0TO15:VDU19,Z,0;0;:NEXT
M$="GET READY!":IFlev%>4:M$="OH BOY...!":IFlev%>8:M$="AAAAAAAGH!"
VDU19,14,7;0;17,14,31,5,15:PRINT;M$;:PROCtune2:ENDPROC
...
IF(lev%MOD4)=1:PROCc(40,980,40,3):PROCmt(864,200,64,2):PROCmt(980,200,64,2)
IF(lev%MOD4)=2:PROCc(40,860,40,1):PROCmt(864,180,32,15):PROCmt(980,200,64,15)
IF(lev%MOD4)=3:PROCc(100,920,40,7):PROCc(120,920,40,15):PROCmt(864,180,32,7):PROCmt(980,200,64,7)
IF(lev%MOD4)=0:PROCc(40,860,40,5):PROCmt(864,120,32,1):PROCmt(980,200,64,1)
...
ENDPROC
DEFPROCdeadfish(E%,C%):P%=P%-1:L%=L%-1:SOUND3,2,96,4:PROCd(F%?E%,FX%?E%,FY%?E%):F%?E%=F%?E%+4:PROCd(F%?E%,FX%?E%,FY%?E%):FH%?E%=2:FV%?E%=0:PROCstatus:VDU18,3,7,25,4,BX%?C%*8;BY%?C%*4;251:BX%?C%=255:BY%?C%=255:ENDPROC
...
DEFPROCtune2:chan%=1:RESTORE138:FORu=1TO7:READnote,dur:dur=dur+2:SOUNDchan%,5,note,dur
FORT=1TO400:NEXT
