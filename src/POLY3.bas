...
DEFPROCA:RESTORE(113+(l%MOD4)):READv:VDU28,0,4,19,0,17,v,12,26:READci%,m%:IFci%=1READw,x,y,z:PROCc(w,x,y,z)
...
PRINTB$:FORT=242TO247:VDU23,T:FORI=1TO8:VDURND(255):NEXT,:FORy=127TO160STEP32:FORx=0TO1279STEP64:VDU18,0,3,25,4,x;y;RND(6)+241:NEXT,:FORG=0TO9:h=RND(2)+194:VDU18,0,15,25,4,(G*136)+8;194;h,10,8,194,18,0,2,25,4,G*136;194;h,10,8,194:NEXT
FORG=3TO6:VDU18,0,5,25,4,64+(G*136);192;197:NEXT:ENDPROC
DEFPROCa:D%=32:e%=214:f%=1:PROCd(f%,D%,e%):P%=8:L%=8:O%=1224:M%=300:V%=1:g%=1:H%=0:K%=0:IF(l%MOD2)=1cr%=198:co%=1:ELSEcr%=199:co%=5
...
IFq%?E%<>0:IFl%>4:IFFNfb(k%?E%,n%?E%)PROCu
E%=(E%+1)MOD8:ENDPROC
DEFPROCE:OS%=SX%:OY%=SY%:OA%=SD%:IFSX%<D%ANDSX%<62SX%=SX%+V%:SD%=5:IFFNsbr(SX%+12)PROCu
IFSX%>D%ANDSX%>1SX%=SX%-V%:SD%=4:IFFNsb(SX%+4)PROCu
IFSX%=D%SD%=8:IFFNsb(SX%+8)PROCu
IFSY%<e%ANDSY%<194SY%=SY%+V%
IFSY%>e%ANDSY%>64SY%=SY%-V%
IFOS%<>SX%OROY%<>SY%PROCd(OA%,OS%,OY%):PROCd(SD%,SX%,SY%)
ENDPROC
...
DEFPROCB:PRINTD$;P%;:IF(l%MOD2)=1ORl%>8:VDU31,6,30,17,6,209,32,17,7:PRINT;L%
VDU17,128,5:ENDPROC
...
IFO%<1068:IFH%=0ANDK%=0SOUND2,3,240,2:VDU18,3,6,25,4,612;850;207;:K%=1
TIME=0:ENDPROC
...
PROCp:PROCw:ENDPROC
DEFPROCM:B=0:FORA=0TO7:IFq%?A<>2ANDn%?A<>0SOUND2,1,(30+B*10),1:S%=S%+50:B=B+1:PROCe:VDU18,0,1,25,4,k%?A*16;(n%?A*4)+40;208
...
UNTILFALSE:ENDPROC
DEFPROCw:TIME=0:REPEATUNTILTIME=200:ENDPROC
...
DEFFNsb(a%):IFSX%<D%ORa%>(D%+12)ORSY%<(e%-12)ORSY%>e%:=FALSE:ELSE=TRUE
