...
IFFNrs(CX%,CY%)=5 ORFNrs(CX%+1,CY%)=5 ORFNrs(CX%+2,CY%)=5 ORFNrs(CX%+3,CY%)=5 OD%=50:SF%=2:PROChurt ELSEOD%=200
...
IFC%<8:P%=P%-1:S%=S%+200:VDU4:PRINTTAB(13-LENSTR$S%,0);S%:SOUND1,1,99,1:VDU5,18,3,7,25,4,BX%?C%*8;BY%?C%*4;251:SOUND0,-10,150,1:BX%?C%=255:BY%?C%=255
PROCstatus:ENDPROC
DEFPROCfishcheck(X%,Y%,E%):CALLQ%:C%=?&AB2
IFC%<8:PROCdeadfish(E%):VDU18,3,7,25,4,BX%?C%*8;BY%?C%*4;251:SOUND0,-10,150,1:BX%?C%=255:BY%?C%=255
ENDPROC
DEFPROCdeadfish(E%):P%=P%-1:L%=L%-1:SOUND0,-10,50,10:PROCdraw(F%?E%,FX%?E%,FY%?E%):F%?E%=F%?E%+4:PROCdraw(F%?E%,FX%?E%,FY%?E%)
