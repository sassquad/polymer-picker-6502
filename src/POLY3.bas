...
DEFPROCa:D%=32:e%=214:f%=1:PROCd(f%,D%,e%):P%=8:L%=8:pl%=248+l%MOD4:O%=1224:M%=300:V%=1:g%=1:E%=0:ee%=0:H%=0:K%=0:ST%=0:IF(l%MOD2)=1cr%=228:co%=1:ELSEcr%=229:co%=5
...
ENDPROC
DEFPROCJ:O%=1224:SOUND1,1,80,1:PRINTA$:H%=1:K%=0:V%=1:PRINTE$:ENDPROC
...
DEFPROCj:FORo=0TO3:U%?o=(o+1)*16:zz%?o=(RND(101)-1)+96:PROCd(9,U%?o,zz%?o):NEXT:ENDPROC
DEFPROCcj:ee%=(ee%+1)MOD4:IFFNu(U%?ee%,zz%?ee%)=FALSE:PROCu
ENDPROC
DEFPROCd(A%,X%,Y%)CALLW%:ENDPROC
DEFFNf(c%,d%):IFc%<D%ORc%>(D%+12)ORd%<(e%-12)ORd%>e%:=FALSE:ELSE=TRUE
DEFFNt(a%):IFa%<D%ORa%>(D%+12)ORSY%<(e%-12)ORSY%>e%:=FALSE:ELSE=TRUE
