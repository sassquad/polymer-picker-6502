...
IFSD%=8IF(SX%+4)>(D%+4)SX%=SX%-V%:ELSE:IFSX%>(D%+6)SX%=SX%-V%
IFSD%=8IF(SX%+4)<(D%+4)SX%=SX%+V%:ELSE:IF(SX%+4)<(D%+4)SX%=SX%+V%
IFSX%>(D%+8)SD%=4:IFFNs(SX%+4)PROCu
IF(SX%+8)<(D%+4)SD%=5:IFOA%=8:SX%=SX%-8:IFFNb(SX%+12)PROCu
IF(SX%+8)>D%IFSX%<(D%+8)SD%=8:IFOA%=5:SX%=SX%+4:IFFNs(SX%)PROCu
...
DEFFNb(a%):IFa%<D%ORSX%>(D%+12)ORSY%<(e%-12)ORSY%>e%:=FALSE:ELSE=TRUE
DEFFNs(a%):IFa%<D%ORa%>(D%+12)ORSY%<(e%-12)ORSY%>e%:=FALSE:ELSE=TRUE
